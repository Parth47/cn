...
int main(void)
{
	...
	/*if ((recv_len = recvfrom(s, fname, 20, 0, (struct sockaddr *) &si_other, &slen)) == -1)
	{
		die("recvfrom()");
	}*/
	char fna[100];
	//strcpy(fna,"/home/krishna/ftp new/");	//replace krishna by current username
	//strcat(buf,"_new");
	strcpy(fna,buf);
	//printf("------%s",buf);
	memset(buf,0,503);
	...
	/*if ((recv_len = recvfrom(s, buf, (mm%503), 0, (struct sockaddr *) &si_other, &slen)) == -1)
	{
		die("recvfrom()");
	}*/
...
gescoe@ubuntu:~/Desktop$ gcc udpfileServer.c 
gescoe@ubuntu:~/Desktop$ ./a.out

Enter Filename with extension: udp.txt
